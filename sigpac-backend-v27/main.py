...
async def _cargar_estaciones_aemet() -> list:
    """Descarga y cachea el catálogo de estaciones AEMET."""
    global _aemet_estaciones, _aemet_estaciones_cargadas
    if _aemet_estaciones_cargadas:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            for e in estaciones:
                try:
                    ...
                    # Formato AEMET: "412429N" o "41.4081"
        ...
        logger.error(f"Error cargando estaciones AEMET: {ex}")
        return []
...
async def clima_parcela(
    ...
    if not indicativo:
        raise HTTPException(status_code=503, detail="No se encontraron estaciones AEMET")
